...
class MultiFactorBacktest:
    ...
    def resolve(self, filename):
        """在候选目录中查找输入文件；找不到时返回首候选路径（调用方会报出清晰错误）。"""
        for d in self.search_dirs:
            p = Path(d) / filename
            if p.exists():
                return p
        return Path(self.search_dirs[0]) / filename
    ...
    def load_factor_csv(self, filename, factor_name):
        ...
        df = pd.read_csv(self.resolve(filename))
    ...
    def load_data(self):
        ...
        def latest_factor(pattern):
            for d in self.search_dirs:
                ...
                if dates:
                    return pattern.replace("*", sorted(dates)[-1])
                # 回退: 仓库内的无日期后缀版本（如 流动性因子等权和.csv）
                undated = Path(d) / pattern.replace("*", "")
                if undated.exists():
                    return undated.name
    ...
    def plot_results(self, save_path=None):
        ...
        factor_names_cn = {
            ...
            "volatility": "波动率因子",
            "liquidity": "流动性因子",
            self.dev_key: f"{self.model}定价偏差因子"
        }
    ...
    def print_performance_summary(self):
        ...
        # 保存到CSV
        save_path = self.data_dir / f"{self.model_dash}_alpha_strategy_metrics.csv"
...
def main(model="BS"):
    """主函数"""
    print("\n" + "=" * 60)
    print(f"6因子复合策略回测系统 ({model})")  
    print("=" * 60)

    # 初始化（输出到本脚本目录，输入文件自动在仓库内解析）
    backtest = MultiFactorBacktest(model=model)
    ...
    # 绘图
    backtest.plot_results(save_path=backtest.data_dir / f"{backtest.model_dash}_alpha_strategy_chart.png")

    # 保存结果
    backtest.save_results(backtest.data_dir / f"{backtest.model_dash}_alpha_strategy_results.csv")
...
if __name__ == "__main__":
    main(model=sys.argv[1].upper() if len(sys.argv) > 1 else "BS")
